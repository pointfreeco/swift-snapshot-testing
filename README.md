...
> ⚠️ Warning: By default, Xcode will try to add the SnapshotTesting package to your project's application/framework target. Please ensure that SnapshotTesting is added to a _test_ target instead, as documented in the last step, below.

 1. From the **File** menu, navigate through **Swift Packages** and select **Add Package Dependency…**.
 2. Enter package repository URL: `https://github.com/pointfreeco/swift-snapshot-testing.git`
 3. Confirm the version and let Xcode resolve the package
 4. On the final dialog, update SnapshotTesting's **Add to Target** column to the test target that will contain snapshot tests (if you have more than one test target, you can add SnapshotTesting to them manually by linking it in a build phase)
