...
> ⚠️ Warning: Snapshots must be compared using a simulator with the same OS, device gamut, and scale as the simulator that originally took the reference to avoid discrepancies between images. To maintain snapshots for several such simulators side by side — with the recording platform tracked in each snapshot's filename — see [Multi-Simulator Snapshots](Documentation/Multi-Simulator-Snapshots.md).
