import Foundation
  ...
  final class WindowPool {
    ...
    // MARK: - Private methods
      ...
      private func windowSceneThroughTemporaryWindow(_ sessionRole: UISceneSession.Role)
        -> UIWindowScene?
      {
        let window = UIWindow()
        ...
        if window.windowScene?.session.role != sessionRole && application == nil {
          #if os(visionOS)
            print(
              "Failed to create UIWindow for requested UISceneSession.Role (\(sessionRole.rawValue))."
            )
            print(
              "This may be unexpected behavior. If tests fail, try running them through an Xcode project."
            )
          #endif
        }
