  ...
  extension Snapshotting where Value == UIBezierPath, Format == UIImage {
    ...
    public static func image(
      precision: Float = 1, perceptualPrecision: Float = 1, scale: CGFloat = 1, format imgFormat: ImageFormat
    ) -> Snapshotting {
      return SimplySnapshotting.image(
        precision: precision, perceptualPrecision: perceptualPrecision, scale: scale, format: imgFormat
      ).pullback { path in
