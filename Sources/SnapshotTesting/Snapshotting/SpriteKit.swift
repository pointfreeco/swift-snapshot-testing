  ...
  import SpriteKit
    ...
    extension Snapshotting where Value == SKScene, Format == NSImage {
      ...
      public static func image(precision: Float = 1, perceptualPrecision: Float = 1, size: CGSize, format: ImageFormat)
        -> Snapshotting
      {
        return .skScene(precision: precision, perceptualPrecision: perceptualPrecision, size: size, format: format)
      }
    }
  #elseif os(iOS) || os(tvOS)
  ...
  extension Snapshotting where Value == SKScene, Format == Image {
    fileprivate static func skScene(precision: Float, perceptualPrecision: Float, size: CGSize, format: ImageFormat)
      -> Snapshotting
    {
