  ...
  private func compare(_ old: UIImage, _ new: UIImage, precision: Float) -> Bool {
    ...
    if let newContext = context(for: newCgImage, bytesPerRow: minBytesPerRow),
    ...
    guard
      let newerContext = context(for: newerCgImage, bytesPerRow: minBytesPerRow, data: &newerBytes)
    else { return false }
  ...
  private func context(for cgImage: CGImage, bytesPerRow: Int, data: UnsafeMutableRawPointer? = nil)
    -> CGContext?
  {
    guard
