  ...
  extension Diffing where Value == UIImage {
    ...
    public static func image(
      precision: Float = 1, perceptualPrecision: Float = 1, scale: CGFloat? = nil, format: ImageFormat
    ) -> Diffing {
    ...
    -> String?
  {
    ...
    guard
      let imgData = EncodeImage(image: new, format),
      let newerCgImage = UIImage(data: imgData)?.cgImage,
