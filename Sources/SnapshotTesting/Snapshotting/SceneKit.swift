  ...
  import SceneKit
    ...
    extension Snapshotting where Value == SCNScene, Format == NSImage {
      ...
      public static func image(precision: Float = 1, perceptualPrecision: Float = 1, size: CGSize, format: ImageFormat)
        -> Snapshotting
      {
        return .scnScene(precision: precision, perceptualPrecision: perceptualPrecision, size: size, format: format)
      }
    }
  #elseif os(iOS) || os(tvOS)
  ...
  extension Snapshotting where Value == SCNScene, Format == Image {
    fileprivate static func scnScene(precision: Float, perceptualPrecision: Float, size: CGSize, format: ImageFormat)
      -> Snapshotting
    {
