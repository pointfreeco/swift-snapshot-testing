...
final class InlineSnapshotManager: @unchecked Sendable {
  ...
  private func _records(for filePath: StaticString) -> [InlineSnapshot] {
    ...
    for (snapshotURL, snapshot) in _snapshots where snapshotURL.absoluteString.starts(with: url.absoluteString) {
      records.append(snapshot)
    }
    return records
  }

  private func _writeInlineSnapshots(_ snapshots: [InlineSnapshot], at url: SnapshotURL, testSource: TestSource) {
    for snapshot in snapshots {
      ...
      let snapshotRewriter = SnapshotRewriter(
        ...
        snapshots: snapshots.sorted {
          $0.line != $1.line
          ? $0.line < $1.line
          : $0.closureDescriptor.trailingClosureOffset < $1.closureDescriptor.trailingClosureOffset
        },
