...
struct InlineSnapshotEngine<Executor: SnapshotExecutor>: SnapshotEngine where Executor.Output: BytesRepresentable {
  ...
  init(
    ...
    InlineSnapshotObservation.shared.registerIfNeeded()
  ...
  func sourceURL(
    ...
    using tester: SnapshotTester<InlineSnapshotEngine<Executor>>
  ) throws -> URL {
...
private extension InlineSnapshotEngine {
  ...
  func missing(_ context: SnapshotFailContext) -> String {
    ...
    let failure: String
    if closureDescriptor.trailingClosureLabel == SnapshotClosureDescriptor.defaultTrailingClosureLabel {
